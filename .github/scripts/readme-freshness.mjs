...
const MODULE_ROOTS = ["app", "worker"];
const MARKER_NAME = "vibeshare-module-fingerprint";
...
function moduleReadmes(files) {
  return files.filter(
    (file) =>
      isReadme(file) &&
      MODULE_ROOTS.some(
        (root) => file === `${root}/README.md` || file.startsWith(`${root}/`),
      ),
  );
}
...
function ownedFiles(readme, files, readmes) {
  return files.filter(
    (file) => !isReadme(file) && ownerOf(file, readmes) === readme,
  );
}
...
function fingerprint(readme, files, readmes, hashes) {
  ...
  hash.update("vibeshare-readme-fingerprint-v1\0");
...
function selectedReadmes(root, targets, readmes) {
  ...
  return [
    ...new Set(targets.map((target) => normalizeTarget(root, target, readmes))),
  ];
}
...
function changedFilesSinceReadme(root, readme, readmes) {
  ...
  const changed = splitNullTerminated(
    runGit(
      ["diff", "--name-only", "-z", baseline, "--", moduleDirectory(readme)],
      root,
    ),
  );
...
function check(root, selected, files, readmes, hashes, options = {}) {
  ...
  for (const readme of selected) {
    ...
    if (recorded.fingerprint !== actual) {
      ...
      if (changed.length === 0) {
        console.log(
          "            module content no longer matches the reviewed fingerprint",
        );
      }
      needsReview += 1;
      continue;
  ...
  console.log(
    `README freshness: ${current} current, ${needsReview} requiring review`,
  );
...
function review(root, selected, files, readmes, hashes) {
  ...
  console.log(
    `Updated ${selected.length} README fingerprint${selected.length === 1 ? "" : "s"}.`,
  );
}
...
Modules are tracked README.md files below app/ and worker/. A source file
...
function main() {
  ...
  if (readmes.length === 0) {
    fail(`no tracked module READMEs found below ${MODULE_ROOTS.join(", ")}`);
  }
...
try {
  ...
  console.error(
    `README freshness failed: ${error instanceof Error ? error.message : String(error)}`,
  );
  process.exitCode = 1;
}
